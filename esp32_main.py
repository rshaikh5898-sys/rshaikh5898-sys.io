...
# ─── SERVER & WIFI CONFIG ─────────────────────────────────────────────────────
WIFI_SSID    = "YOUR_WIFI_NAME_HERE"        # <--- TYPE YOUR WIFI NAME HERE
WIFI_PASSWORD= "YOUR_WIFI_PASSWORD_HERE"    # <--- TYPE YOUR WIFI PASSWORD HERE
...
def get_device_token():
    ...
    try:
        ...
        print("[Auth] Server unreachable (OSError):", e)
        return False
...
def transcribe_audio(raw_pcm, num_samples):
    ...
    try:
        ...
        print("[STT] Server unreachable (OSError):", e)
        return ""
    except Exception as e:
        print("[STT] Error:", e)
        return ""
...
def ask_ai(text):
    ...
    try:
        ...
        print("[AI] Server unreachable (OSError):", e)
        return "", ""
    except Exception as e:
        print("[AI] Error:", e)
        return "", ""
...
def download_audio(audio_url):
    ...
    try:
        ...
        print("[Audio] Server unreachable (OSError):", e)
        return None
    except Exception as e:
        print("[Audio] Error:", e)
        return None
...
def main():
    ...
    if not connect_wifi(WIFI_SSID, WIFI_PASSWORD):
        print("[Boot] Failed to connect to WiFi! Restarting in 5s...")
        time.sleep(5)
        machine.reset()
    ...
    # Send a ping every 30 seconds to show "Online" in the web dashboard
    ping_counter = 0

    while True:
        ping_counter += 1
        if ping_counter >= 300:   # 300 x 100ms = 30 seconds
            ping_counter = 0
            try:
                # Simple ping to update device_last_seen
                res = urequests.get(SERVER_URL + "/api/wifi/status", headers={"X-Device-Token": DEVICE_TOKEN}, timeout=5, ssl_params={"cert_reqs": 0})
                res.close()
            except:
                pass
